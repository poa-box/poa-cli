/**
 * Shared EligibilityModule read layer for the vouch + role domains.
 * Port of src/commands/vouch/helpers.ts (plus the subgraph list reads from
 * src/commands/vouch/list.ts and src/commands/role/applications.ts): functions
 * take the GraphClient / provider explicitly instead of importing
 * process-global singletons.
 *
 * Both domains read the same module, whose admin surface is superAdmin-gated.
 * Facts VERIFIED against contracts origin/main src/EligibilityModule.sol:
 *
 ...
 *   detect the ambiguity and refuse. See graph/documents/vouch.ts for both tiers.
...
import { resolveOrgId, resolveOrgModules, requireModule } from './resolve';
import { createReadContract } from '../contracts';
import { tryAggregate, Call, CallResult } from '../multicall';
import {
  FETCH_VOUCH_CONFIG,
  FETCH_VOUCH_STATUS,
  FETCH_VOUCH_STATUS_EPOCH_AWARE,
  FETCH_VOUCHES_FOR_ORG,
} from '../graph/documents/vouch';
import {
  FETCH_ALL_ROLE_APPLICATIONS,
  FETCH_USER_ROLE_APPLICATIONS,
} from '../graph/documents/role';
import { formatRelativeTime } from '../format';
import { CliError, PreconditionError } from '../errors';
import { EXIT } from '../exit-codes';
import { VOUCH_FLAG_ENABLED, VOUCH_FLAG_COMBINE_HIERARCHY } from '../perms';

// Vouch flag bit rules live in ../perms (browser-pure seed shared with the
// frontend); re-exported here so eligibility consumers get them with the rest
// of the read layer.
export { VOUCH_FLAG_ENABLED, VOUCH_FLAG_COMBINE_HIERARCHY };

/**
 * Resolve an org to its EligibilityModule address.
 * Port of src/commands/vouch/helpers.ts resolveEligibilityModule.
 */
export async function resolveEligibilityModule(
  client: GraphClient,
  orgIdOrName: string | undefined,
  chainId?: number
): Promise<{ orgId: string; eligibilityModuleAddress: string }> {
...
export function decodeVouchConfig(raw: {
  quorum: ethers.BigNumberish;
  membershipHatId: ethers.BigNumberish;
  flags: ethers.BigNumberish;
}): VouchConfigView {
 ...
 * (multicall.ts degrades to parallel provider.call on chains without the
 * canonical deployment, so callers always get one result per call).
 *
...
/** What `pop vouch config show` reads: the hat config + the org-wide rate limit. */
export interface VouchConfigStatus {
  config: VouchConfigView;
  /** getMaxDailyVouches() — ALWAYS RPC, no subgraph field exists. */
  maxDailyVouches: number;
  source: 'subgraph' | 'rpc';
}

/**
 * Port of `pop vouch config show` — src/commands/vouch/config.ts.
 * Config is SUBGRAPH-FIRST (display only) with getVouchConfig() as fallback;
 * `preferSubgraph: false` (the CLI's explicit --rpc) pins the config read to
 * the provider. getMaxDailyVouches() has no subgraph field and stays on chain.
 */
export async function readVouchConfigStatus(
  client: GraphClient,
  provider: ethers.providers.Provider,
  eligibilityModuleAddress: string,
  hatId: ethers.BigNumber,
  opts: { chainId?: number; preferSubgraph?: boolean } = {}
): Promise<VouchConfigStatus> {
  const contract = createReadContract(eligibilityModuleAddress, 'EligibilityModuleNew', provider);
  const [subgraphConfig, maxDailyVouches] = await Promise.all([
    opts.preferSubgraph !== false
      ? fetchVouchConfigFromSubgraph(client, eligibilityModuleAddress, hatId, opts.chainId)
  ...
  return {
    ...
    source: subgraphConfig ? 'subgraph' : 'rpc',
  };
}
 ...
 *
 * Two tiers (graph/documents/vouch.ts): the epoch mirror when the deployment indexes
 * it — which is exact, and lets a reconfigured hat still be served from the subgraph —
 * and the legacy active-row count, which can only refuse to answer once an epoch bump
 * is suspected. `queryWithFieldFallback` drops to the legacy tier on the unknown-field
 ...
 *
 * `preferSubgraph: false` (set when the caller pinned an explicit RPC node)
 ...
 * tx would have produced. Port of `pop vouch for`'s gateVouch —
 * src/commands/vouch/for.ts.
 */
 ...
 * applications` reads the subgraph via listRoleApplications below.
 */
...
// ---------------------------------------------------------------------------
// Subgraph list reads (display only)
// ---------------------------------------------------------------------------

/** One row of FETCH_VOUCHES_FOR_ORG (raw subgraph entity shape). */
export interface VouchRow {
  ...
  vouchCount: string | null;
  isActive: boolean;
  createdAt: string | null;
}

/**
 * Port of `pop vouch list` — src/commands/vouch/list.ts. Pure subgraph
 * (FETCH_VOUCHES_FOR_ORG); display only, never a quorum decision (see the
 * document's isActive epoch note in graph/documents/vouch.ts). `hatId` filter
 * is applied client-side by string equality, exactly as the CLI does.
 */
export async function listVouches(
  client: GraphClient,
  eligibilityModuleAddress: string,
  opts: { hatId?: string; chainId?: number } = {}
): Promise<VouchRow[]> {
  const result = await client.query<{ vouches: VouchRow[] }>(
    FETCH_VOUCHES_FOR_ORG,
    { eligibilityModuleId: eligibilityModuleAddress },
    opts.chainId
  );
  ...
  if (opts.hatId) {
    vouches = vouches.filter((v) => v.hatId === opts.hatId);
  }
  return vouches;
}

/** One row of FETCH_ALL/USER_ROLE_APPLICATIONS (raw subgraph entity shape). */
...
/**
 * Port of `pop role applications` — src/commands/role/applications.ts.
 * Pure subgraph: FETCH_USER_ROLE_APPLICATIONS when `applicant` is given
 * (lowercased, as the CLI does), else FETCH_ALL_ROLE_APPLICATIONS — both
 * filtered by eligibilityModuleId. `hatId` filter is applied client-side by
 * string equality, exactly as the CLI does.
 */
export async function listRoleApplications(
  client: GraphClient,
  eligibilityModuleAddress: string,
  opts: { applicant?: string; hatId?: string; chainId?: number } = {}
): Promise<RoleApplicationRow[]> {
  const result = opts.applicant
    ? await client.query<{ roleApplications: RoleApplicationRow[] }>(
        FETCH_USER_ROLE_APPLICATIONS,
        { eligibilityModuleId: eligibilityModuleAddress, applicant: opts.applicant.toLowerCase() },
        opts.chainId
      )
    : await client.query<{ roleApplications: RoleApplicationRow[] }>(
        FETCH_ALL_ROLE_APPLICATIONS,
  ...
  let applications = result.roleApplications || [];
  if (opts.hatId) {
    applications = applications.filter((a) => a.hatId === opts.hatId);
  }
  return applications;
}
 ...
 * Port of `pop vouch for`'s resolveRoleToHatId — src/commands/vouch/for.ts —
 * minus the TTY disambiguation select (CLI UX): exact match first, then
 * partial; multiple matches always fail with the candidate list.
 */
export async function resolveRoleNameToHatId(
  client: GraphClient,
  orgIdOrName: string,
  roleName: string,
  chainId?: number
  ...
  const results = await client.queryAllChains<any>(query, {});
