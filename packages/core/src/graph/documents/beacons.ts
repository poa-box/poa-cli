...
import type { GraphClient, FieldFallbackTier } from '../client';
...
export async function fetchOrgBeaconSnapshot(
  ...
  const { data, tierIndex } = await client.queryWithFieldFallback<any>(tiers, { chainId });
...
export async function fetchImplementationVersionIndex(
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    const { data, tierIndex: tier } = await client.queryWithFieldFallback<any>(tiers, { chainId });
    tierIndex = tier;
