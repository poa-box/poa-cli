 ...
 *      Requires the caller to pass `orgId` AND a `client` (the injected
 *      GraphClient); without either the EIP-1967 slot walk runs exactly as
 *      before. The implementation is still fetched FROM the beacon over RPC,
 *      because this function gates writes (see
...
import { fetchOrgBeaconSnapshot, beaconAddressFromSnapshot } from './graph/documents/beacons';
import type { OrgBeaconSnapshot } from './graph/documents/beacons';
...
export interface DetectFeaturesOptions {
  /**
   * Org id. When supplied (together with `client`), the proxy's BEACON address
   * comes from the subgraph (RegisteredContract.beacon) instead of an EIP-1967
   * slot read; the implementation is still fetched from that beacon over RPC.
  ...
  /**
   * Injected GraphClient for the beacon-snapshot read. Core never constructs
   * one itself; without a client the subgraph shortcut is skipped and the
   * EIP-1967 slot walk runs — identical to skipSubgraph.
   */
  client?: GraphClient;
  /**
   * Alternative injection: a snapshot fetcher. Takes precedence over `client`.
   * Hosts that already wrap the snapshot read in their own module (the CLI's
   * src/queries/beacons, which tests mock) inject that wrapper here so the
   * seam stays interceptable.
   */
  fetchBeaconSnapshot?: (orgId: string, chainId: number) => Promise<OrgBeaconSnapshot>;
}
 ...
 *
 * Returns null when the beacon is not indexed (or no GraphClient was injected)
 * — the caller then walks the EIP-1967 slots exactly as before.
 */
async function beaconImplementationViaSubgraph(
  ...
  if (!opts.client && !opts.fetchBeaconSnapshot) return null;
  try {
    const snapshot = opts.fetchBeaconSnapshot
      ? await opts.fetchBeaconSnapshot(opts.orgId, chainId)
      : await fetchOrgBeaconSnapshot(opts.client!, opts.orgId, chainId);
 ...
 *   implementation: subgraph (needs `opts.orgId` + `opts.client`) → EIP-1967
 *                   slot walk
