...
export const selfMetricsHandler = {
  ...
  handler: async (argv: ArgumentsCamelCase<SelfMetricsArgs>) => {
    ...
    console.log('');
    // Derive agent name from POP_AGENT_NAME env or HOME basename; fallback "Agent".
    const agentName = process.env.POP_AGENT_NAME
      || (homedir().split('/').pop() || 'Agent');
    const titleName = agentName.charAt(0).toUpperCase() + agentName.slice(1);
    console.log(`  ${titleName} Self-Metrics`);
    console.log('  ══════════════════');
