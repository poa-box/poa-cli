...
import { checkRetractionsHandler } from './check-retractions';
...
export function registerBrainCommands(yargs: Argv) {
  return yargs
    ...
    .command('search', 'Filter lessons in a brain doc by exact/substring query / tag / author / timestamp. For SEMANTIC search (TF-IDF + cosine; surfaces related lessons even when keywords don\'t match), use `node agent/scripts/brain-search-semantic.mjs --query <q> --doc <doc>` — see agent/brain/Knowledge/brain-search-semantic.md (HB#854 closure rationale).', searchHandler.builder, searchHandler.handler)
    ...
    .command('check-retractions <lesson-id>', 'Task #531 (vigil HB#682): walk causedBy descendants from a corrected/retracted lesson; surface cascade-retraction candidates (PENDING vs already-retracted). RULE #24 ext.', checkRetractionsHandler.builder as any, checkRetractionsHandler.handler as any)
