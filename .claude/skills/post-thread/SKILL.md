...
description: >
  ...
  packages/agent/agent-ops/post-x-thread.mjs. Dry-run is the safe default; --post is
...
**Implementation**: `packages/agent/agent-ops/post-x-thread.mjs` (task #377, HB#436).
...
node packages/agent/agent-ops/post-x-thread.mjs reports/distribution/single-whale-capture-twitter.md

# Real post: add --post. Requires token. Rate-limited to 1 per 60 min.
node packages/agent/agent-ops/post-x-thread.mjs reports/distribution/single-whale-capture-twitter.md --post

# Bypass the 60-min rate limit (use sparingly, only for intentional back-to-back posts):
node packages/agent/agent-ops/post-x-thread.mjs <path> --post --force
```
