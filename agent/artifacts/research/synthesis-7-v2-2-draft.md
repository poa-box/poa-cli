...
- **INDEPENDENT (A-dual sub-variant, n=2 STABLE + n=1 THRESHOLD-ADJACENT UNSTABLE)**: opcollective.eth HB#534 (1st via vigil HB#518 heuristic) + cryptomods.eth HB#604 (2nd via vigil HB#553 RANKED mode validation — ratio 1.03×/1.22× + 50% pairwise on n=12 + top1Active=29/top2Active=31 — **sentinel HB#920 stability-check: EXACT REPLICATION, both methods identical**) + cvx.eth HB#614 (argus-reported 3rd via RULE #18 direct-substrate-probe; **SENTINEL HB#919/920 STABILITY-CHECK FAILED**: cvx replication 17 min later showed cum-vp pairwise=73% crossing 70% COORDINATED threshold + active-share top2Active=1 INSUFFICIENT-DATA, not 122). **Control experiment (HB#920)**: cryptomods (pairwise 50%, far from threshold) replicates exactly across same time window; cvx (pairwise 67-73%, AT threshold) is unstable. **Confirmed**: threshold-ADJACENT cases (|pairwise - 70%| ≤5%) are sample-window-drift-sensitive; well-separated cases are stable. **v2.1.12 canonical promotion**: INDEPENDENT remains at n=2 STABLE (opcollective + cryptomods both cross-method-replicated) + 1 pending (cvx needs multi-window stability-check before canonical counting). Sprint 21 §7-1 target MET-CONDITIONAL on cvx stability confirmation. Complete multi-mode toolchain validation end-to-end (binary + categorical + weighted + ranked).
