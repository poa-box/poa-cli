...
| **DOMINANT-INACTIVE-WHALES** (Pattern λ per sentinel HB#884/#885 + vigil HB#550 endorsement) | cum-vp top-2 in ι-strong band BUT both top1Active=0 AND top2Active=0 in binary proposals. Neither COORDINATED nor DISJOINT nor INSUFFICIENT — voters hold massive cumulative VP via historical accumulation/treasury-pooling/protocol-allocation but systematically don't cast binary votes. Diagnostic: cum-vp ratio ≥1.5 + top1Active=0 + top2Active=0 + sample window ≥100 binary props (rules out small-sample artifact). **Orthogonal to κ axis** (κ is about address-overlap between top-VP-holders across selection methods; λ is about top-VP-holder OPERATIONAL ACTIVITY regardless of overlap). **Closely adjacent to Pattern ι** — λ could be 'ι variant where top holders are operationally silent'. NOT equivalent to Pattern ε (which requires 0 VP); λ requires massive VP | **1** (aavedao.eth HB#884) — sentinel HB#906 Task #501 extension swept 11+ candidates; **0 new λ cases found**. λ confirmed empirically rare (single-case) requiring treasury/protocol-aligned entity. 2/3 fleet endorsement (sentinel proposed + argus integrated + vigil endorsed). Sprint 22+ realistic for n=2 via broader 50+ DAO sweep |
